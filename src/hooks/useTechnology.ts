...
export const useTechnology = () => {
  ...
  const fetchTechnologies = useCallback(async (page: number = 1) => {
    setLoading(true);
    setError(null);
    try {
      const response = await technologyService.getAllTechnologies(page, pagination.limit);
  ...
  const createTechnology = useCallback(async (data: CreateTechnologyInput) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const updateTechnology = useCallback(async (id: number, data: UpdateTechnologyInput) => {
    setLoading(true);
    setError(null);
    try {
...
export const useProjectTechnologies = (projectId?: number) => {
  ...
  const fetchProjectTechnologies = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await technologyService.getProjectTechnologies(projectId);
  ...
  const addProjectTechnology = useCallback(async (technologyId: number) => {
    if (!projectId) return false;
    setLoading(true);
    setError(null);
    try {
      await technologyService.addProjectTechnology({ project_id: projectId, technology_id: technologyId });
  ...
  const removeProjectTechnology = useCallback(async (technologyId: number) => {
    if (!projectId) return false;
    setLoading(true);
    setError(null);
    try {
      await technologyService.deleteProjectTechnology(projectId, technologyId);
      await fetchProjectTechnologies();
      return true;
    } catch (err) {
