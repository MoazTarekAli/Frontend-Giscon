...
const ProjectTechnologyManager: React.FC<ProjectTechnologyManagerProps> = ({
  projectId,
}) => {
  const {
    projectTechnologies,
  ...
  const handleRemoveTechnology = async (technologyId: number) => {
    if (
      window.confirm(
        'Are you sure you want to remove this technology from the project?'
      )
    ) {
  ...
  const availableTechnologies = technologies.filter(
    (tech) =>
      !projectTechnologies.some((pt) => pt.technology_id === tech.technology_id)
  );
